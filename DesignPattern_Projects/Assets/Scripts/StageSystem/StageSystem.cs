...
public class StageSystem : IGameSystem
{
    ...
    private void InitPosition()
    {
        ...
        go = GameObject.Find("TargetPosition");
        if (go != null)
        {
            //go.SetActive(false);
    ...
    private void InitStageChain()
    {
        ...
        handler1.SetNextHandler(handler2).SetNextHandler(handler3).SetNextHandler(handler4).SetNextHandler(handler5)
            .SetNextHandler(handler6).SetNextHandler(handler7).SetNextHandler(handler8).SetNextHandler(handler9);
