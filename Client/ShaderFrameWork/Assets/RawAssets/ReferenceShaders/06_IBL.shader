...
///---My Image Based Lighting Function      
            ...
            float3 IBL_Approx(samplerCUBE EnvMap,float3 SpecularColor,float3 N,float3 L,float3 V,float Roughness)
            {
                ...
                float NdotV = saturate(dot(N,V));
                return Li_u * EnvBRDFApprox(SpecularColor,Roughness,NdotV);
            }
...
///---Fragment Function
		...
		///Compute Color
            ...
            ///IBL
                ...
                float3 IBLColor = IBL_Approx(_EnvMap,specColor,normalDir,lightDir,viewDir,roughness) * (dot(normalDir,lightDir)*0.5f + 0.5f);
