	...
	SubShader
	{
		...
		Pass
		{
			...
			struct v2f
			{
				...
				float3 normal : NORMAL;//法线向量坐标
				float4 texcoord : TEXCOORD0;//一级纹理坐标
			...
			fixed4 frag(v2f IN) : COLOR
			{
				//【1】先准备好需要的参数
				//获取纹理颜色
				float4 texColor = tex2D(_MainTex, IN.texcoord.xy);
				//获取法线的方向
				//float3 worldNormal = normalize(IN.normal);
                //获取切空间法线
				fixed3 bump = UnpackNormal(tex2D(_BumpMap, IN.texcoord.zw));
				//使用矩阵乘法获得世界空间法线信息
