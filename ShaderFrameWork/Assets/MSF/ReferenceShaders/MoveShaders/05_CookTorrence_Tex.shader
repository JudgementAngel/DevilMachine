Shader "Move/05_CookTorrence_Tex"
{
	Properties
	{
		...
		_STex("_S Tex Metallic(r)Roughness(b) ",2D) = "black"{}
...
///---Fragment Function
		...
		///Define variables
				...
				float3 normalDir = mul(normalTex, TangentToWorld);
		...
		///Uniform variables
				float4 SCol = tex2D(_STex, i.uv * _MainTex_ST.xy + _MainTex_ST.zw);
				float metallic = SCol.r;
				float roughness = max(0.001f, SCol.b);
